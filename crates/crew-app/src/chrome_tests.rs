...
fn inputbar_rect_spans_action_area() {
    ...
    // card_bottom(800, 8) = 800 - 8 = 792 (the frame stretches to its rect,
    // so there is no cell remainder to quantize away); h = 3*20; y = 732.
...
fn sidebar_and_inputbar_bottoms_align() {
    ...
    // floored bottom border above the input bar's bottom. Both frames now
    // stretch to their rects, so the RECTS' bottoms are the drawn bottoms,
    // and they must be the same pixel row.
