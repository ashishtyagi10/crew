...
impl TodoPane {
    ...
    /// Enter in the composer: a lone `@tag`/`#name` sets that axis's filter
    ...
    pub(crate) fn submit(&mut self) {
        ...
        if self.editing.is_none() {
            if let Some((sigil, name)) = parse::lone_tag(trimmed) {
                let axis = match sigil {
                    parse::WHO => &mut self.who,
                    _ => &mut self.filter,
                };
                *axis = (!name.is_empty()).then(|| name.to_string());
                self.reset_input();
                self.scroll = 0;
                self.sel = None;
                return;
    ...
    pub(crate) fn cycle_filter(&mut self, forward: bool) {
        let tags = super::tagmenu::known_tags(&self.items, parse::PROJECT);
        if tags.is_empty() {
            return;
        }
