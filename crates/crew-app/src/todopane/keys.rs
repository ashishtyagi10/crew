...
//! (`sel` is `Some` — its keys live in [`super::listkeys`]). The tag popup,
//! when open, wins arrows/Tab/Enter first; its other keys fall through to
//! composer editing so typing keeps narrowing the query.
...
pub(crate) fn apply(
    ...
    if let Some(m) = &mut p.tagmenu {
        match input {
            ...
            Tab | Enter => {
                if let Some(tag) = m.matches.get(m.sel).cloned() {
                    let sigil = m.sigil;
                    p.input = tagmenu::accept(&p.input, sigil, &tag);
