...
impl CrewApp {
    ...
    pub(crate) fn pane_at_sidebar(&self) -> Option<usize> {
        ...
        let (sb, ch, l) = self.nav_hit_geometry()?;
...
/// `PANES` header sits at content row `panes_top`, and pane `k` directly under
/// it. `None` for the border, the header, and everything above.
