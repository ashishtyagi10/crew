...
/// A selection, and the bold it puts in the file — with no marker on screen
/// either before or after.
#[test]
#[ignore = "needs a GPU adapter; writes PNGs"]
fn doc_shot_selection() {
    let _g = crate::app::theme_test_guard();
    let mut view = doc(MD);
    view.start_editing(80);
    // Into the paragraph and select a phrase inside it: emphasis is inline,
    // so a selection that left the block would be refused (and the shot that
    // did exactly that is why it is refused).
    for _ in 0..40 {
        view.move_caret(crate::viewpane::caret::Step::Right, 80, 24);
    }
    view.anchor_here();
    ...
    let (w, h) = (720u32, 300u32);
    for (name, bold) in [("doc-selection", false), ("doc-bolded", true)] {
        if bold {
            view.wrap_selection("**", 80, 24);
        }
        let px = crate::shotdraw_tests::draw(w, h, 13.0, |cw, ch| {
            let m = 12.0;
            let rect = Rect {
