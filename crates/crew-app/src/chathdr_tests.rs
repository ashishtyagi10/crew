...
fn header_is_status_only_no_title() {
    ...
    let cells = header_cells(60, "general", true, false, None, false, 0);
...
fn reductionist_header_has_no_msg_or_turn_chatter() {
    ...
    let line = text(&header_cells(60, "general", true, false, None, false, 0), 0);
    assert!(
        !line.contains("msg"),
...
fn connecting_dot_when_disconnected() {
    let line = text(&header_cells(60, "", false, false, None, false, 0), 0);
    assert!(line.contains('\u{25cb}'), "connecting dot missing: {line}");
}

#[test]
fn awaiting_shows_thinking_spinner() {
    let line = text(&header_cells(60, "c", true, true, None, false, 0), 0);
...
fn header_carries_no_token_meter() {
    ...
    assert!(!text(&header_cells(60, "c", true, false, None, false, 0), 0).contains("tok"));
}
...
fn esc_interrupt_hint_absent_when_idle() {
    let line = text(&header_cells(60, "c", true, false, None, false, 0), 0);
    assert!(
...
fn esc_interrupt_hint_is_first_dropped_when_narrow() {
    let wide = text(&header_cells(60, "c", true, true, None, false, 0), 0);
    ...
    let narrow = text(&header_cells(24, "c", true, true, None, false, 0), 0);
    assert!(
...
fn compact_chip_appears_when_compact_on_a_wide_pane() {
    let line = text(&header_cells(60, "c", true, false, None, true, 0), 0);
...
fn compact_chip_is_dropped_before_the_esc_hint_when_narrow() {
    ...
    let wide = text(&header_cells(60, "c", true, true, None, true, 0), 0);
    ...
    let narrow = text(&header_cells(20, "c", true, true, None, true, 0), 0);
...
#[test]
fn the_interrupt_hint_counts_the_tool_calls_in_flight() {
    let line = text(&header_cells(60, "c", true, true, None, false, 2), 0);
    assert!(
        line.contains("\u{00b7} 2 tools running \u{00b7} esc interrupts"),
        "{line:?}"
    );
