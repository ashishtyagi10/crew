...
/// Chars of the user's request carried into the brief; mirrors `route::TASK_CAP`.
...
pub(super) fn combine(
    ...
    // Plan order, not completion order, so the same plan briefs the same way
    // twice. A task the graph no longer knows (re-planned in) keeps its id.
    let title = |id: TaskId| {
        graph
            .get(id)
    ...
    let parts: Vec<(String, String)> = parts.into_iter().map(|(_, t, o)| (t, o)).collect();

    emit(activity("thinking", "user"))?;
    let line = match call(&prompt(goal, &parts)) {
        Ok(text) if !text.trim().is_empty() => {
            let text = text.trim().to_string();
            crate::broker::sessionlog::append(ANSWER_SENDER, &text);
            text
        }
        Ok(_) => "could not combine the workers' answers: the model returned nothing".into(),
        Err(e) => format!(
...
fn activity(state: &str, from: &str) -> PluginEvent {
