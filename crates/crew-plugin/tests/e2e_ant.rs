...
use common::{messages, roster_signins, run_broker_paced, seed_specialists, unique_dir};
...
fn a_signed_in_ant_profile_serves_with_a_minted_bearer() {
    ...
    let events = run_broker_paced(
        ...
        &[(&send("/model"), 800), (&send("say hello please"), 4000)],
    );
    let msgs = messages(&events);
    let all: String = msgs.iter().map(|(s, t)| format!("{s}: {t}\n")).collect();

    // The picker's listing and the roster's sign-in rows both read the
    // CLI's verdict.
    assert!(
        all.contains("1. anthropic \u{2014} signed in \u{00b7} OAuth profile via its CLI"),
        "{all}"
    );
    let ant = roster_signins(&events)
        .into_iter()
        .find(|o| o.name == "anthropic")
        .expect("the roster offers anthropic as a sign-in row");
...
/// but signed out, so `/model` says exactly what to run (grayed,
/// unnumbered), `/model anthropic` says it too, and `/logout anthropic`
/// hands the sign-out to the CLI instead of deleting anything.
#[cfg(unix)]
#[test]
fn a_signed_out_ant_is_the_sign_in_affordance() {
    ...
    // Picked by name while signed out: the command to run, and the way
    // back is `/model anthropic` — not a construct that no longer exists.
    assert!(
        all.contains("run `ant auth login`, then /model anthropic again here"),
        "{all}"
    );
    assert!(
