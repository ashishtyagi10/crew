...
fn cell_geometry_is_a_square_grid_about_six_dots_to_a_row() {
    let (spacing, _) = ModernPaper::cell_geometry(36.0); // retina, ~14pt
...
fn dots_never_touch_their_neighbours() {
    for cell_h in [12.0f32, 20.0, 28.0, 36.0, 44.0, 72.0, 200.0] {
        let (spacing, radius) = ModernPaper::cell_geometry(cell_h);
...
fn pitch_rides_the_cell_and_clamps_at_the_extremes() {
    assert_eq!(ModernPaper::cell_geometry(48.0).0[0], 8.0);
    assert_eq!(ModernPaper::cell_geometry(36.0).0[0], 6.0);
    // Tiny cell: 12/6 = 2 would alias — floor at 5.
    assert_eq!(ModernPaper::cell_geometry(12.0).0[0], 5.0);
    // Huge cell: 200/6 = 33 would read as polka dots — ceiling at 9.
    assert_eq!(ModernPaper::cell_geometry(200.0).0[0], 9.0);
}

#[test]
fn radius_is_a_fifth_of_the_pitch_within_its_own_clamps() {
    assert_eq!(ModernPaper::cell_geometry(36.0).1, 1.2);
    assert_eq!(ModernPaper::cell_geometry(12.0).1, 1.0); // pitch 5 → 1.0
    assert_eq!(ModernPaper::cell_geometry(200.0).1, 1.4); // pitch 9 → 1.8, capped
}
