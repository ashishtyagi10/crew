...
fn fs(in: VsOut) -> @location(0) vec4<f32> {
  ...
  // too) and nothing else: the sheet stays whole under the words and stops
  // at its own edge, a fieldset's legend. Clearing the fill under them left
  // each title in a dark hole; raising the sheet into a tab over them read
  // as a folder tab stuck on the card.
